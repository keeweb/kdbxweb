...
function setDate(node, date) {
    setText(node, date ? date.toISOString().replace(dateRegex, '') : undefined);
}
...
function strToBoolean(str) {
    return str && str.toLowerCase() === 'true' ? true : str && str.toLowerCase() === 'false' ? false :
        str.toLowerCase() === 'null' ? null : undefined;
}
 ...
 * @param {Node} node
 */
function protectedPlainValues(node) {
    traverse(node, function(node) {
...
module.exports.protectedPlainValues = protectedPlainValues;
