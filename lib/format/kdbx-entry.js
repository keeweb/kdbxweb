 ...
 * @returns {KdbxEntry}
 */
KdbxEntry.create = function(meta) {
    var entry = new KdbxEntry();
    entry.uuid = KdbxUuid.random();
    entry.icon = Consts.Icons.Key;
    entry.times = KdbxTimes.create();
    entry.fields.Title = meta.memoryProtection.title ? '' : ProtectedValue.fromString('');
    entry.fields.UserName = meta.memoryProtection.userName ? meta.defaultUser||'' : ProtectedValue.fromString(meta.defaultUser||'');
    entry.fields.Password = meta.memoryProtection.password ? '' : ProtectedValue.fromString('');
    entry.fields.URL = meta.memoryProtection.url ? '' : ProtectedValue.fromString('');
    entry.fields.Notes = meta.memoryProtection.notes ? '' : ProtectedValue.fromString('');
    // TODO: set autoType
    return entry;
};
