 ...
 * @param {boolean} [forSaveAsXml=false] - true, if the xml will be used for export
 */
KdbxMeta.prototype.write = function(parentNode, forSaveAsXml) {
    ...
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.DbNameChanged), this.nameChanged);
    XmlUtils.setText(XmlUtils.addChildNode(node, XmlNames.Elem.DbDesc), this.desc);
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.DbDescChanged), this.descChanged);
    XmlUtils.setText(XmlUtils.addChildNode(node, XmlNames.Elem.DbDefaultUser), this.defaultUser);
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.DbDefaultUserChanged), this.defaultUserChanged);
    ...
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.DbKeyChanged), this.keyChanged);
    ...
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.RecycleBinChanged), this.recycleBinChanged);
    XmlUtils.setUuid(XmlUtils.addChildNode(node, XmlNames.Elem.EntryTemplatesGroup), this.entryTemplatesGroup);
    this.kdbx._setXmlDate(XmlUtils.addChildNode(node, XmlNames.Elem.EntryTemplatesGroupChanged), this.entryTemplatesGroupChanged);
    ...
    if (forSaveAsXml || this.kdbx.header.versionMajor < 4) {
 ...
 * @returns {KdbxMeta}
 */
KdbxMeta.create = function(kdbx) {
    var now = new Date();
    var meta = new KdbxMeta(kdbx);
 ...
 * @return {KdbxMeta}
 */
KdbxMeta.read = function(xmlNode, kdbx) {
    var meta = new KdbxMeta(kdbx);
