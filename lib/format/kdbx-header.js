...
var HeaderNames = [
    'EndOfHeader',
    'Comment', 'CipherID', 'CompressionFlags', 'MasterSeed', 'TransformSeed',
    'TransformRounds', 'EncryptionIV', 'ProtectedStreamKey', 'StreamStartBytes', 'InnerRandomStreamID',
    'KdfParameters', 'PublicCustomData'
];
...
var HeaderConst = {
    ...
    MaxHeaderFieldByVersion: { 3: 10, 4: 12 },
    AesCipherUuid: new KdbxUuid('McHy5r9xQ1C+WAUhavxa/w=='),
    SupportedRandomAlgorithms: [Consts.CrsAlgorithm.Salsa20, Consts.CrsAlgorithm.ChaCha20]
};
...
KdbxHeader.prototype._readInnerRandomStreamID = function(bytes) {
    var id = new DataView(bytes).getUint32(bytes, true);
    if (HeaderConst.SupportedRandomAlgorithms.indexOf(id) < 0) {
        throw new KdbxError(Consts.ErrorCodes.Unsupported, 'crsAlgorithm ' + id);
    }
...
KdbxHeader.prototype.write = function(stm) {
    ...
    var maxHeaderField = HeaderConst.MaxHeaderFieldByVersion[this.versionMajor];
    for (var i = 1; i <= maxHeaderField; i++) {
...
KdbxHeader.create = function() {
    ...
    header.dataCipherUuid = HeaderConst.AesCipherUuid;
