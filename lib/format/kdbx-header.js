...
KdbxHeader.prototype._writeBinary = function(stm) {
    ...
    for (var binaryId = 0; binaryId < binaryIds.length; binaryId++) {
        stm.setUint8(HeaderConst.InnerHeaderBinaryFieldId);
        var binary = this.kdbx.binaries[binaryId];
        if (!binary) {
            throw new KdbxError(Consts.ErrorCodes.FileCorrupt, 'no binary ' + binaryId);
        }
...
KdbxHeader.prototype._writeField = function(stm, headerId, fields) {
    var headerField = fields[headerId];
    if (headerField) {
 ...
 * @param {BinaryStream} stm
 */
KdbxHeader.prototype.writeInnerHeader = function(stm) {
    this._validateInner();
    for (var id = 1; id < InnerHeaderFields.length; id++) {
        this._writeField(stm, id, InnerHeaderFields);
    }
