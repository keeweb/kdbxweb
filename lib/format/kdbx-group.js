 ...
 * @param {function} callback - will be invoked with entry argument
 */
KdbxGroup.prototype.forEachEntry = function(callback) {
    this.entries.forEach(function(entry) { callback(entry); });
    this.groups.forEach(function(group) { group.forEachEntry(callback); });
};
