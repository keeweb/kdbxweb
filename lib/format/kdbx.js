...
Kdbx.create = function(credentials, name) {
    ...
    kdbx.header = KdbxHeader.create(kdbx);
    kdbx.meta = KdbxMeta.create(kdbx);
...
Kdbx.prototype.merge = function(remote) {
    ...
    // TODO: this is incorrect! Fix it, we cannot merge binaries this way!
...
Kdbx.prototype._parseMeta = function() {
    ...
    this.meta = KdbxMeta.read(node, this);
};
...
Kdbx.prototype._resolveEntryFieldRefs = function(entry) {
    for (var binaries = Object.keys(entry.binaries), i = 0; i < binaries.length; i++) {
        ...
        if (binary && binary.ref && this.binaries[binary.ref]) {
            binary.value = this.binaries[binary.ref];
        }
    }
};

Kdbx.prototype._buildXml = function(forSaveAsXml) {
    var xml = XmlUtils.create(XmlNames.Elem.DocNode);
    this.meta.write(xml.documentElement, forSaveAsXml);
    var rootNode = XmlUtils.addChildNode(xml.documentElement, XmlNames.Elem.Root);
    this.groups.forEach(function(g) { g.write(rootNode, this); }, this);
    var delObjNode = XmlUtils.addChildNode(rootNode, XmlNames.Elem.DeletedObjects);
    this.deletedObjects.forEach(function (d) { d.write(delObjNode, this); }, this);
    this.xml = xml;
};

Kdbx.prototype._setXmlDate = function(node, dt) {
    var isBinary = this.header.versionMajor >= 4;
