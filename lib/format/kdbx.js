...
/**
 * Remove extra history, it it doesn't match defined rules, e.g. there are too many records
 */
Kdbx.prototype.cutHistory = function() {
    var historyMaxItems = this.meta.historyMaxItems;
    //var historyMaxSize = this.meta.historyMaxSize; // TODO: delete binaries
    if (historyMaxItems && historyMaxItems > 0) {
        this.getDefaultGroup().forEachEntry(function (entry) {
            if (entry.history.length > historyMaxItems) {
                entry.history.splice(0, entry.history.length - historyMaxItems);
            }
        });
    }
};
