 ...
 * @param {ArrayBuffer} data - xml file contents
 * @param {KdbxCredentials} credentials
 * @param {Function} callback (called with loaded Kdbx)
 */
Kdbx.import = function(data, credentials, callback) {
    if (!(data instanceof ArrayBuffer)) {
        throw new KdbxError(Consts.ErrorCodes.InvalidArg, 'data');
    }
    if (!(credentials instanceof KdbxCredentials)) {
    ...
    kdbx.credentials = credentials;
    kdbx.header = KdbxHeader.create();
    kdbx._readXml(data, function(err) {
        if (err) {
            return callback(null, err);
        }
        try {
            kdbx._protectedPlainValues();
            kdbx._loadFromXml();
        } catch (err) {
            return callback(null, err);
        }
...
Kdbx.prototype._readXml = function(data, callback) {
    try {
...
Kdbx.prototype._protectedPlainValues = function() {
    XmlUtils.protectedPlainValues(this.xml.documentElement);
};
